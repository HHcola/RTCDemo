...
import io.agora.ag_faceu.model.Message;
import io.agora.ag_faceu.model.User;
import io.agora.faceu.CameraLoader;
import io.agora.faceu.DemoConstants;
import io.agora.faceu.FuCameraCompat;
import io.agora.faceu.FuCore;
import io.agora.faceu.GPUImageRenderer;
import io.agora.faceu.GPUVideoViewDecorator;
import io.agora.faceu.HardCodeData;
import io.agora.ag_faceu.R;
import io.agora.ag_faceu.model.AGEventHandler;
import io.agora.ag_faceu.model.ConstantApp;
...
public class ChatActivity extends BaseActivity implements AGEventHandler, IHeadsetPlugListener, GPUImageFilterGroupBase.IGroupStateChanged {
    ...
    private final HashMap<Integer, VideoViewDecorator> mUidsList = new HashMap<>(); // uid = 0 || uid == EngineConfig.mUid
    ...
    private void doUpdateFaceDetector() {
        ...
        renderer.setOnProcessedFrameListener((byte[] data, int width, int height, long timestamp, Rotation rotation) -> {
            worker().deliverVideoFrame(data, width, height, rotation.asInt(), timestamp);
        });
    }
    ...
    protected void initUIandEvent() {
        ...
        mUidsList.put(0, mGPUVideoView); // get first surface view
    ...
    protected void deInitUIandEvent() {
        ...
        if (mGridVideoViewContainer != null) {
            mGridVideoViewContainer.removeAllViews();
        }
    ...
    private SurfaceView getLocalView() {
        for (HashMap.Entry<Integer, VideoViewDecorator> entry : mUidsList.entrySet()) {
            if (entry.getKey() == 0 || entry.getKey() == config().mUid) {
                return entry.getValue().mSurfaceView;
            }
        }
    ...
    private void doRenderRemoteUi(final int uid) {
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                VideoViewDecorator decorator = new VideoViewDecorator(getBaseContext());
                decorator.attachView(surfaceV);
                mUidsList.put(uid, decorator);
    ...
    public void onJoinChannelSuccess(String channel, final int uid, int elapsed) {
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                VideoViewDecorator local = mUidsList.remove(0);
    ...
    private void switchToSmallVideoView(int bigBgUid) {
        HashMap<Integer, VideoViewDecorator> slice = new HashMap<>(1);
    ...
    private void bindToSmallVideoView(int exceptUid) {
        ...
        log.debug("bindToSmallVideoView " + twoWayVideoCall + " " + (exceptUid & 0xFFFFFFFFL) + " " + create);
