...
public class SenseTimeDetector implements Runnable, IFaceDetector {
    ...
    void handleDetect(ByteBuffer sampleData, @SdkConstants.NotationClockwiseDirection int direction) {
        ...
        if (null == mFaceTrack) {
            mFaceTrack = new STMobileMultiTrack106(mContext, STMobileMultiTrack106.FACE_KEY_POINTS_COUNT, new AuthCallback() {
                @Override
                public void authErr(String err) {
                    Log.e(TAG, "authErr: " + err);
                }
            });
